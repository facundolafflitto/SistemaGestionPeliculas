...
import { FaFilm, FaTv, FaSignOutAlt, FaChartPie } from "react-icons/fa";

const tabs = [
  { name: "Películas", key: "peliculas", icon: <FaFilm /> },
  { name: "Series",    key: "series",    icon: <FaTv /> },
  { name: "Dashboard", key: "dashboard", icon: <FaChartPie /> }, // 👈 nuevo
];
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`relative flex items-center gap-2 px-5 py-2 rounded-full font-medium text-sm transition-all
                ${seccion === tab.key ? "text-white" : "text-gray-400 hover:text-white"}`}
